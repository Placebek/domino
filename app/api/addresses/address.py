...
from app.api.addresses.commands.address_crud import create_address_with_relations, get_all_cities, get_districts_by_city, get_street_by_district
...
async def create_address_view(data: AddressCreate, access_token: str = Depends(get_access_token), db: AsyncSession = Depends(get_db)):
    return await create_address_with_relations(data=data, access_token=access_token, db=db)
