...
const schema = Yup.object().shape({
  first_name: Yup.string().required('Атыңызды жазу керек'),
  last_name: Yup.string().required('Тегіңізді жазу керек'),
  email: Yup.string().email('Дұрыс email жазу керек').required('Email жазу керек'),
...
function onSubmit(values, { setErrors }) {
  ...
  const { phone_number, password, first_name, last_name, email } = values

  return authStore
    .register(phone_number, password, first_name, last_name, email)
...
<template>
  <div class="relative h-screen flex justify-center items-center bg-domino-login">
    <div class="bg-blue-300/95 shadow-xl w-[70vw] absolute max-h-[45vh] rounded-2xl pb-[2vh]">
      <h2 class="flex text-center justify-center text-[3vh] font-semibold mt-[3vh]">
        Жаңа аккаунт жасау
      </h2>
      <Form @submit="onSubmit" :validation-schema="schema" v-slot="{ errors, isSubmitting }">
        ...
        <div>
          <div class="flex items-center justify-center mt-[1vh]">
            <div class="text-[1.3vh]">Егер сізде аккаунт бар болса:&nbsp</div>
            <router-link to="/login" class="text-blue-500 underline font-bold text-[1.4vh]">
              Кіру
            </router-link>
          </div>
        </div>
